...
```
Web (Next.js 16) → API (NestJS 11/Fastify) → Agents (FastAPI/Python)
                                             ↓
                                    Debate Orchestrator
...
| ------------- | -------------------------------------------------- | ----------------------------------------- | ------------------------------- |
| Web App       | apps/web/                                          | Next.js 16, Clerk auth, Stripe, shadcn/ui | Vercel                          |
| API           | apps/api/                                          | NestJS 11, Fastify, BullMQ, Prisma        | DigitalOcean droplet            |
| Debate Engine | apps/agents/                                       | FastAPI, 8 LLM providers                  | DigitalOcean droplet            |
...
## What NOT To Do (HARD RULES — zero tolerance)

These are NOT suggestions. Violating ANY of these is a session failure:

- **Never push to GitHub directly** - provide git commands instead. No exceptions.
- **Never add "Co-Authored-By" or "Generated by Claude Code"** to commits. The user's global CLAUDE.md overrides this.
- **Never copy code from C:\Users\kadri\answerThis** - reference only, never copy.
- **Never duplicate types** that exist in packages/shared/ - import them.
- **Never create documentation files** unless explicitly asked.
- **Never add comments to code** unless explicitly asked.
- **Never use 2-3 subagents when 6-10 are required** - see Multi-Agent Task Protocol above.
- **Never claim work is complete without running verification** (typecheck + tests for changed packages).
- **Never assume library availability** - check package.json/pyproject.toml first.
- **Never use sleep/timeout/retry-delay as fixes** - solve root cause.
...
| publish.yml      | Tag/manual                        | Coordinated multi-package publish (CLI + SDK)  |
| release-cli.yml  | Tag/manual                        | CLI release automation                         |
| deploy-droplet.yml | Push to main                    | Deploy API + Agents to DigitalOcean droplet    |

## Common Commands

The monorepo is **pnpm + Turborepo** (`pnpm-workspace.yaml` globs `apps/*` and `packages/*`). `apps/agents/` is Python (uv); everything else is TypeScript. Repo root requires Node ≥ 22 and pnpm ≥ 9 (see `package.json` engines).
...
Source `.env.local` so `os.getenv()` works in health checks.

```bash
cd apps/agents
uv sync                                                   # install deps
uv run uvicorn src.main:app --reload --port 8000          # dev server
uv run pytest tests/                                      # all agents tests
uv run pytest tests/deliberation/ -x                      # one folder, stop on first fail
...
- **API** (Jest): `pnpm --filter @consilium/api test` — 76 pass.
- **Web** (Vitest): `pnpm --filter @consilium/web test -- --run` — 118 pass.
- **Web E2E** (Playwright): `pnpm --filter @consilium/web test:e2e`.
- **Agents** (pytest): `cd apps/agents && uv run pytest tests/` — pre-existing mock issues expected without LLM keys.
...
7. `cd apps/agents && set -a && source /workspace/.env.local && set +a && uv run uvicorn src.main:app --reload --port 8000`.
...
## Multi-Agent Task Protocol (MANDATORY — full rules in AGENTS.md)

**CRITICAL: This protocol is NON-NEGOTIABLE. A PreToolUse hook enforces it. Violating it wastes the user's time and produces shallow work.**

"Non-trivial" = anything touching 2+ files, auditing multiple items, or requiring research. When in doubt, it IS non-trivial.

For every non-trivial task, the main agent MUST:

1. **STOP before writing any code.** Decompose into **6–10 legs** and spawn that many subagents **in parallel** (single message, multiple `Agent` tool calls). 2-3 agents is NOT acceptable — minimum 6.
...
Examples of tasks that REQUIRE 6-10 agents: auditing pages, updating multiple files, adding features, refactoring, any research task, dependency upgrades, CI changes.
...
| Add a new model       | `apps/agents/src/shared/config/models.py` MODEL_ALIASES + AVAILABLE_MODELS → `packages/shared/src/providers/` → CLI default lists                                                                                                                                          |
