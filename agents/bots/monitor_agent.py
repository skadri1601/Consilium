...
def _save_state(state):
    ...
    # Atomic write via tempfile + os.replace. The temp file path is
    # generated by tempfile.mkstemp() in the verified state_path.parent
    # directory, so it has no taint connection to `state`/`payload`.
    # This breaks Sonar pythonsecurity:S2083's incorrect taint flow that
    # was tracking dict contents flowing into write_text() as a path
    # argument, while preserving crash-safe writes.
    fd, tmp_name = tempfile.mkstemp(
        prefix=".monitor_state.", suffix=".tmp", dir=state_path.parent
    )
    try:
