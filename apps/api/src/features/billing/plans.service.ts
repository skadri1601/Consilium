...
export const MODEL_PRICING: Record<string, { inputPer1MTokensCents: number; outputPer1MTokensCents: number }> = {
...
export class PlansService {
  ...
  calculateModelCostCents(modelId: string, inputTokens: number, outputTokens: number, applyMarkup = false): number {
    const pricing = MODEL_PRICING[modelId];
    if (!pricing) return 0;
    const raw = (inputTokens / 1_000_000) * pricing.inputPer1MTokensCents + (outputTokens / 1_000_000) * pricing.outputPer1MTokensCents;
