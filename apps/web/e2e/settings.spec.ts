...
test.describe("Settings Page", () => {
  ...
  test("should display API key inputs", async ({ page }) => {
    ...
    if (hasApiSection) {
      ...
      await expect(
        page.locator('input[aria-label*="OpenAI"]').first(),
      ).toBeVisible();
      await expect(
        page.locator('label:has-text("Anthropic")').first(),
      ).toBeVisible();
      await expect(
        page.locator('label:has-text("Google")').first(),
      ).toBeVisible();
    }
  });
  ...
  test("should test API key", async ({ page }) => {
    ...
    if (hasApiSection) {
      ...
      await expect(page.locator("text=/Invalid|Valid/").first()).toBeVisible({
        timeout: 5000,
      });
    }
  });
