...
export const metadata: Metadata = buildMetadata({
  ...
  keywords: [
    "consilium typescript sdk",
    "ai debate javascript",
    "llm council node",
  ],
});
...
const methods = [
  {
    ...
    desc: "Check API health. Returns status, version, uptime.",
  },
  {
    name: "deliberate(options)",
    returns: "Promise<DeliberationResult>",
    desc: "Run deliberation. Returns goldenPrompt, dissentReport, cost, auditTrail, votes, confidenceScores.",
  },
  {
    name: "redTeam(options)",
    returns: "Promise<RedTeamReport>",
    desc: "Adversarial assessment. Returns attacks, defenses, judgments, overallScore, vulnerabilityCount.",
  },
  {
    name: "blindEval(options)",
    returns: "Promise<EvaluationResult>",
    desc: "Anonymous evaluation. Returns rankings, scores, method.",
  },
  {
    name: "estimateCost(options)",
    returns: "Promise<CostEstimate>",
    desc: "Pre-calculate cost. Returns estimatedCost, breakdown per model, rounds, mode.",
  },
  {
    name: "streamDeliberation(options)",
    returns: "AsyncIterable<Event>",
    desc: "Real-time SSE stream. Yields event, agentId, chunk, round, model, content.",
  },
];
...
const errorClasses = [
  {
    name: "ConsiliumError",
    status: "any",
    desc: "Base error class. Has message and optional statusCode.",
  },
  {
    name: "AuthenticationError",
    status: "401",
    desc: "Invalid or missing API key / auth token.",
  },
  {
    name: "TimeoutError",
    status: "408",
    desc: "Request exceeded configured timeout.",
  },
  {
    name: "ServerError",
    status: "500+",
    desc: "Server-side error. Automatically retried.",
  },
  {
    name: "RateLimitError",
    status: "429",
    desc: "Rate limit exceeded. Includes retry_after header.",
  },
];
...
const types = [
  {
    ...
    desc: "'quick' | 'council' | 'deep' | 'blind' | 'redteam' | 'jury' | 'market' | 'auto' | 'prediction-market' | 'adversarial' | 'delphi'",
  },
  {
    name: "DeliberationResult",
    desc: "{ goldenPrompt, dissentReport, cost, auditTrail[], votes, confidenceScores }",
  },
  {
    name: "RedTeamReport",
    desc: "{ attacks[], defenses[], judgments[], overallScore, vulnerabilityCount }",
  },
  { name: "EvaluationResult", desc: "{ rankings[], scores, method }" },
  {
    name: "CostEstimate",
    desc: "{ estimatedCost, breakdown[{ model, role, estimatedCost }], rounds, mode }",
  },
  {
    name: "DeliberationEvent",
    desc: "{ event, agentId?, chunk?, round?, model?, content?, message?, data? }",
  },
];
            ...
            <CardContent className="space-y-3">
              ...
              <pre className="rounded-lg bg-bg-1 p-4 text-sm overflow-x-auto space-y-1">
                <code className="text-muted-foreground block">
                  yarn add @myconsilium/sdk
                </code>
                <code className="text-muted-foreground block">
                  pnpm add @myconsilium/sdk
                </code>
              </pre>
            </CardContent>
          </Card>
                ...
                <tbody>
                  {errorClasses.map((e) => (
                    <tr
                      ...
                      className="border-b border-white/[0.06] last:border-0"
                    >
                      <td className="px-4 py-2.5 font-mono text-sm text-dissent">
                        {e.name}
                      </td>
  ...
  switch (event.event) {
    ...
    case"dissent:report":
      ...
      break;
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Error Handling Example</CardTitle>
            </CardHeader>
            <CardContent>
              <pre className="rounded-lg bg-bg-1 p-4 text-sm overflow-x-auto">
