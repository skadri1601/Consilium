...
export const metadata: Metadata = buildMetadata({
  title: "Research",
  description: "The peer-reviewed research behind Consilium's deliberation modes: multi-agent debate, self-consistency, tree-of-thoughts, constitutional AI, and more.",
  path: "/research",
...
const papers = [
  {
    ...
    venue: "ICML 2024",
    venueColor: "bg-warm/12 text-warm border-warm/20",
    ...
    findings: ["Multi-agent debate improves factual accuracy by 8-15% across benchmarks","GSM8K math reasoning improved from 82% to 91% with 3-agent debate","MMLU scores improved 8-12% compared to single-model baselines","Debate is most effective on questions requiring multi-step reasoning","Improvement scales with agent count and rounds, diminishing after 3-4 rounds",
    ],
  ...
  },
  {
    ...
    venueColor: "bg-warm/12 text-warm-bright border-warm/20",
    ...
    findings: ["Truth has a natural advantage in structured debate — truthful positions are easier to defend","Even asymmetric debates (strong vs. weak model) converge on correct answers","Structured protocols prevent persuasive but incorrect arguments from dominating","Validates debate as a scalable oversight method for AI alignment","Judges improve accuracy when evaluating debate transcripts vs. direct answers",
    ],
  ...
  },
  {
    ...
    venue: "ACL 2024",
    venueColor: "bg-agree/14 text-agree border-agree/30",
    ...
    findings: ["3-10% improvement over the best individual model across reasoning benchmarks","Confidence-weighted voting outperforms simple majority voting by 5-7%","Diverse model ensembles (different architectures) perform better than same-model ensembles","Round-table format enables models to learn from each other's reasoning strategies","Optimal performance at 3-5 models; beyond 5, diminishing returns",
    ],
  ...
  },
  {
    ...
    findings: ["Debate enables evaluation of AI outputs on tasks beyond the judge's direct capability","Adversarial structure incentivizes surfacing the strongest evidence for each position","Debate scales better than direct human oversight for complex tasks","Optimal play in debate converges on truth under reasonable verification assumptions","The approach provides a natural mechanism for identifying and preserving minority opinions",
    ],
  ...
  },
  {
    ...
    findings: ["Structured multi-model discussion produces 23% more creative outputs (human evaluation)","Discussion format encourages exploration of unconventional approaches","Role-play assignment increases creative diversity by forcing perspective shifts","Collaborative refinement improves both novelty and quality simultaneously","Models build on each other's ideas in ways single models cannot self-generate",
    ],
  ...
  },
  {
    ...
    venueColor: "bg-dissent/14 text-dissent border-dissent/30",
    ...
    findings: ["Debate protocols can be optimized for cost without sacrificing safety guarantees","Complexity-based routing reduces cost by 60-80% on simple questions","Efficient debate maintains the quality benefits of full debate at lower cost","Practical implementations can route questions to appropriate debate depth automatically","The doubly-efficient property makes debate viable for production-scale systems",
    ],
    ...
    <div className="min-h-screen">
      <section className="pt-28 pb-16 border-b border-white/[0.08]">
        <div className="container-narrow">
          ...
          <h1 className="display text-[clamp(40px,6vw,72px)] leading-[1.02] max-w-[900px]">
            The <em>peer-reviewed</em> science<br />behind the council.
          </h1>
          <p className="mt-6 max-w-[560px] text-[17px] leading-[1.55] text-ink-secondary">
            Every Consilium feature maps to a specific finding from published research at ICML, ACL, AAAI, and the AI safety community.
          </p>
        </div>
      </section>
                    ...
                    <ul className="space-y-1.5">
                      {paper.findings.map((finding) => (
                        <li key={finding} className="flex items-start gap-2 text-sm text-muted-foreground">
                          <ArrowRight className="h-3.5 w-3.5 mt-0.5 shrink-0 text-warm" />
                          <span>{finding}</span>
                        </li>
                      ))}
                    </ul>
      ...
      <section className="container mx-auto px-4 pb-24">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-2xl border border-white/[0.08] bg-bg-1 p-8">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {implementationMapping.map((row) => (
                    <tr key={row.finding} className="border-b border-white/[0.08]">
