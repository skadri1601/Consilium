...
const ONBOARDED_FLAG_PATH = path.join(os.homedir(), '.consilium', 'onboarded.json');
...
export async function runOnboarding(): Promise<boolean> {
  ...
  console.log(`  ${st.dim('Multiple AI models from different providers debate to solve your task.')}`);
  console.log('');
  ...
  if (rootInfo.isGitRepo || detected.length > 0) {
    ...
    if (rootInfo.isGitRepo) console.log(`    ${st.brand('•')} git repo at ${rootInfo.root}`);
    if (detected.length) console.log(`    ${st.brand('•')} ${detected.join(' / ')}`);
    if (workspace.hasTests) console.log(`    ${st.brand('•')} test suite present`);
    console.log('');
  }
  ...
  if (hasCodebasePermission(rootInfo.root) !== true) {
    ...
    const answer = await ask(`  Allow codebase read for ${rootInfo.root}? [y/N] `);
    if (answer === 'y' || answer === 'yes') {
      grantCodebasePermission(rootInfo.root, 'always');
      console.log(`  ${st.success('Granted.')} ${st.dim('Stored in ~/.consilium/permissions.json')}`);
    } else {
      console.log(`  ${st.dim('Skipped. The council will run without file tools until you grant.')}`);
    }
    console.log('');
  }
