...
function colorForLevel(level: string): (s: string) => string {
  switch (level) {
    ...
    case 'WARN': return st.warning;
    case 'DEBUG': return st.dim;
...
function buildTimeline(debate: any): LogEntry[] {
  ...
  if (debate.status === 'processing' || debate.status === 'completed' || debate.status === 'failed') {
    timeline.push({
...
export async function logsCommand(debateId: string, options: { level?: string }): Promise<void> {
