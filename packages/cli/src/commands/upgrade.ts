...
async function detectManager(): Promise<UpgradeResult['manager']> {
  ...
  const candidates: Array<{ name: UpgradeResult['manager']; cmd: string; args: string[] }> = [
    { name: 'pnpm', cmd: 'pnpm', args: ['list', '-g', '--depth=0', PACKAGE] },
    { name: 'npm', cmd: 'npm', args: ['ls', '-g', PACKAGE, '--depth=0'] },
    { name: 'yarn', cmd: 'yarn', args: ['global', 'list', '--pattern', PACKAGE] },
...
async function runUpgrade(manager: UpgradeResult['manager']): Promise<boolean> {
  ...
  await execFileAsync(entry[0], entry[1], { timeout: 120_000, maxBuffer: 4 * 1024 * 1024 });
  return true;
}
...
export async function upgradeCommand(options: UpgradeOptions = {}): Promise<void> {
  ...
  if (!latestVersion) {
    console.log(st.warning('  Could not reach the npm registry. Check your network and try again.'));
    return;
  }
  ...
  if (options.check) {
    console.log(st.dim('  Run `consilium upgrade` to install the new version.'));
    return;
  }
  ...
  if (manager === 'unknown') {
    ...
    console.log(st.dim('  If you used the standalone binary or the curl|sh installer, re-run:'));
    console.log(st.dim('    curl -fsSL https://install.myconsilium.xyz | sh'));
    console.log(st.dim('  Otherwise, upgrade manually with your package manager:'));
  ...
  try {
    ...
    if (ok) {
      ...
      console.log(st.dim('  Restart any running Consilium processes to pick up the new version.'));
    } else {
      console.log(st.warning('  Upgrade not supported for this install method.'));
    }
  } catch (err) {
