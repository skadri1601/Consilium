...
export async function debugCommand(debateId: string): Promise<void> {
  ...
  console.log(contentLine(`Models:  ${(debate.models || []).join(', ') || 'N/A'}`));
  console.log(contentLine(`Cost:    ${formatCost(debate.totalCost ?? debate.cost)}`));
    ...
    for (let i = 0; i < debate.rounds.length; i++) {
      ...
      if (round.responses?.length) {
        for (const resp of round.responses) {
          ...
          const truncated = truncateText(rawText.replace(/\n/g, ' '), RESPONSE_TRUNCATE_LENGTH);
  ...
  if (synthesis) {
    ...
    const preview = truncateText(synthesis.replace(/\n/g, ' '), RESPONSE_TRUNCATE_LENGTH * 2);
