...
function applyConvergenceUpdateEvent(
  ...
  if (event.convergence !== undefined) {
    const data = event as DeliberationEvent & { single_correct?: number; deliberation_correct?: number };
...
async function runRemoteBenchmark(
  ...
  try {
    ...
    log('ERROR', 'benchmark_failed', { error: msg, data: { benchmark: options.benchmark } });
    console.log(st.error('Benchmark creation failed: ' + msg));
    process.exit(1);
  }

  log('INFO', 'benchmark_started', {
    data: { benchmarkId: benchmarkRun.id, benchmark: options.benchmark, models, mode, n },
  });
  ...
  try {
    await client.streamBenchmark(benchmarkRun.id, (event: DeliberationEvent) => {
      const outcome = handleBenchmarkStreamEvent(event, progress, useLiveProgress);
      if (outcome === 'error') {
