...
import { userHasStoredProviderKeys, type MaskedProviderKeys } from '../utils/post-login-onboarding';
...
import { restoreRollbackSnapshot, type RollbackSnapshot } from '../utils/rollback';
    ...
    if (session.lastGoldenPrompt) {
      try {
        ...
        if (level === 'deny' || !consumeWritePermission(rootInfo.root)) {
          console.log(st.warning('Write permission denied. Use /permissions status to review policy.'));
          return 'continue';
        }
...
function slashApi(args: string[]): SlashResult {
  ...
  const webUrl = config.webUrl || process.env.CONSILIUM_WEB_URL || DEFAULT_WEB_ORIGIN;
...
async function slashKeys(args: string[]): Promise<SlashResult> {
  ...
  if (sub === 'status') {
    ...
    if (!token) {
      console.log(st.warning('No CLI token. Run /api open or consilium login.\n'));
      return 'continue';
    }
    try {
      ...
      if (has) {
        console.log(st.success('At least one provider key is saved.'), st.dim('Manage at'));
        console.log(st.brand(keysUrl));
      } else {
        console.log(st.warning('No provider keys saved.'), st.dim('Debates can use platform Groq where supported.'));
...
async function slashCodebase(args: string[]): Promise<SlashResult> {
  ...
  if (sub === 'allow' || sub === 'grant') {
    ...
    console.log(ok ? st.success('Codebase read access granted for this project scope.') : st.warning('Not granted.'));
    console.log('');
    return 'continue';
  }
...
async function slashPermissions(args: string[]): Promise<SlashResult> {
  ...
  if (sub === 'revoke-write') {
    ...
    console.log(st.success('Revoked write permission for this project scope.\n'));
    return 'continue';
  }
...
async function slashApply(session: ChatSession): Promise<SlashResult> {
  if (!session.lastGoldenPrompt) {
    console.log(st.warning('No synthesis available to apply. Run a debate first.\n'));
    return 'continue';
  }

  const rootInfo = resolveProjectRoot(process.cwd());
  const parsed = parseEditsFromSynthesis(session.lastGoldenPrompt, rootInfo.root);
  if (parsed.edits.length === 0) {
    console.log(st.warning('No structured edits found in last synthesis.'));
    console.log(st.dim('Expected format: ```consilium-edits with JSON edit entries.\n'));
    return 'continue';
  }
...
async function slashRollback(args: string[]): Promise<SlashResult> {
  ...
  const historyDir = path.join(process.env.HOME || process.env.USERPROFILE || '', '.consilium', 'edit-history');
  ...
  if (!snapshotId) {
    try {
      ...
      for (const entry of entries) {
        ...
        try {
          const snap = JSON.parse(fs.readFileSync(snapshotFile, 'utf-8')) as RollbackSnapshot;
          const files = snap.files.map((f) => f.path).join(', ');
          console.log(st.brand(entry), st.dim(`  ${snap.createdAt}  ${snap.files.length} file(s): ${files}`));
        } catch {
  ...
  try {
    snapshot = JSON.parse(fs.readFileSync(snapshotFile, 'utf-8')) as RollbackSnapshot;
  } catch {
...
async function slashReview(args: string[], session: ChatSession): Promise<SlashResult> {
  ...
  console.log(st.success(`Added ${filePath} to context. Your next debate will review this file.\n`));
  console.log(st.dim(`Tip: ask "Review ${filePath} for bugs, style issues, and improvements"\n`));
  return 'continue';
}

function slashEditHistory(): SlashResult {
  const auditFile = path.join(process.env.HOME || process.env.USERPROFILE || '', '.consilium', 'edit-history', 'audit.jsonl');
