...
async function tryExec(cmd: string, args: string[], cwd?: string): Promise<ExecResult | null> {
  try {
...
export async function debatePrCommand(prRef: string, options: DebatePrOptions = {}): Promise<void> {
  if (!prRef) {
  ...
  const meta = await tryExec('gh', ['pr', 'view', prNumber, '--json', 'title,body,author,baseRefName,headRefName,additions,deletions,files']);
  if (!meta || meta.exitCode !== 0) {
    console.log(st.error(`Could not fetch PR ${prNumber}: ${meta?.stderr || 'unknown error'}`));
    process.exit(1);
  }
  ...
  if (!diff || diff.exitCode !== 0) {
    console.log(st.warning(`  Could not fetch PR diff: ${diff?.stderr || 'unknown error'}`));
  }
  ...
  const diffNote = (diff?.stdout?.length ?? 0) > 60_000 ? '\n...[truncated]' : '';
...
export async function debateIssueCommand(issueRef: string, options: DebateIssueOptions = {}): Promise<void> {
  if (!issueRef) {
  ...
  if (linearMatch) {
    ...
    await debateCommand(topic, { ...options, ticket: issueRef, mode: options.mode ?? 'council' });
    return;
  }

  if (!(await hasCommand('gh'))) {
    console.log(st.error('GitHub CLI (gh) not found and the ref is not a Linear ticket id.'));
    console.log(st.dim('  Install gh: https://cli.github.com  (then: gh auth login)'));
    console.log(st.dim('  Or pass a Linear ticket id like MYC-123 to use --ticket fetching.'));
    process.exit(1);
  }
  ...
  const meta = await tryExec('gh', ['issue', 'view', issueNumber, '--json', 'title,body,author,labels,assignees,state,comments']);
  if (!meta || meta.exitCode !== 0) {
    console.log(st.error(`Could not fetch issue ${issueNumber}: ${meta?.stderr || 'unknown error'}`));
    process.exit(1);
  }
  ...
  const labels = (issueMeta.labels ?? []).map((l) => l.name).join(', ') || 'none';
...
export async function debateFailingCommand(options: DebateFailingOptions = {}): Promise<void> {
...
export async function debateStagedCommand(options: DebateStagedOptions = {}): Promise<void> {
  const root = resolveProjectRoot(process.cwd()).root;
  ...
  if (!staged || staged.exitCode !== 0) {
    console.log(st.error(`git diff --staged failed: ${staged?.stderr || 'unknown error'}`));
    process.exit(1);
  }
  ...
  const branch = await tryExec('git', ['rev-parse', '--abbrev-ref', 'HEAD'], root);
