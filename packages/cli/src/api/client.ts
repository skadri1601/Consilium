  ...
  async createDebate(options: DebateOptions): Promise<{ id: string }> {
    ...
    try {
      ...
      if (options.files?.length)
        body.context = { ...body.context, files: options.files };
      if (options.images?.length)
        body.context = { ...body.context, images: options.images };
      if (options.projectFiles?.length)
        body.context = { ...body.context, projectFiles: options.projectFiles };
  ...
  async streamDebate(
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        reject(
          new Error(
            `Stream timeout after ${Math.round(this.streamTimeout / 1000)}s`,
          ),
        );
      }, this.streamTimeout);
    });
  ...
  async streamDeliberation(
    ...
    return new Promise((resolve, reject) => {
      ...
      eventSource.onmessage = (event: any) => {
        try {
          ...
          if (eventType === "done" || eventType === "deliberation_complete") {
            this.log(
              `Deliberation stream completed after ${eventCount} events`,
            );
            eventSource.close();
            resolve();
          }
      ...
      setTimeout(() => {
        ...
        reject(
          new Error(
            `Deliberation stream timeout after ${Math.round(this.streamTimeout / 1000)}s`,
          ),
        );
      }, this.streamTimeout);
    });
  ...
  async streamBenchmark(
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        reject(
          new Error(
            `Benchmark stream timeout after ${Math.round(this.streamTimeout / 1000)}s`,
          ),
        );
      }, this.streamTimeout);
    });
