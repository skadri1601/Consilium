...
function meta(overrides: Partial<OutputMetadata> & { format: OutputFormat }): OutputMetadata {
  return {
    topic: 'Test Topic',
...
describe('getDefaultFilename', () => {
  it('returns slugified .md for markdown', () => {
    expect(getDefaultFilename('markdown', 'My Cool Topic')).toBe('my-cool-topic.md');
  });
  ...
  it('returns slugified .txt for text', () => {
    expect(getDefaultFilename('text', 'Notes & Ideas!')).toBe('notes-ideas.txt');
  });
...
describe('getFileExtension', () => {
  it('returns .md for markdown', () => expect(getFileExtension('markdown')).toBe('.md'));
  it('returns empty for cursorrules', () => expect(getFileExtension('cursorrules')).toBe(''));
  it('returns .md for claude-md', () => expect(getFileExtension('claude-md')).toBe('.md'));
  it('returns .json for json', () => expect(getFileExtension('json')).toBe('.json'));
  it('returns .txt for text', () => expect(getFileExtension('text')).toBe('.txt'));
});
...
describe('formatOutput', () => {
  ...
  describe('markdown format', () => {
    it('includes topic as heading', () => {
      const result = formatOutput(synthesis, meta({ format: 'markdown', timestamp }));
  ...
  describe('json format', () => {
    it('returns valid JSON', () => {
      const result = formatOutput(synthesis, meta({ format: 'json', timestamp }));
  ...
  describe('claude-md format', () => {
    ...
    it('includes section headers', () => {
      const multiParagraph = 'Para one\n\nPara two\n\nPara three\n\nPara four\n\nPara five\n\nPara six';
      const result = formatOutput(multiParagraph, meta({ format: 'claude-md' }));
