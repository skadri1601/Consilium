...
describe('createDebate', () => {
  ...
  it('throws ApiError on 401', async () => {
    ...
    const client = new ConsiliumClient();
    await expect(client.createDebate({ topic: 'Fail' })).rejects.toThrow(ApiError);
  });
...
describe('listDebates', () => {
  ...
  it('handles array response', async () => {
    const debates = [{ id: '1', topic: 'A' }, { id: '2', topic: 'B' }];
...
describe('cancelDebate', () => {
  it('sends POST to cancel endpoint', async () => {
    ...
    const [url, init] = mockFetch.mock.calls[0]!;
    expect(url).toBe('https://test-api.example.com/api/v1/debates/debate-abc/cancel');
    expect(init.method).toBe('POST');
  });
...
describe('createDeliberation', () => {
  ...
  it('throws ApiError on failure', async () => {
    ...
    const client = new ConsiliumClient();
    await expect(
      client.createDeliberation('Fail'),
...
describe('createRedTeam', () => {
  it('sends correct POST body', async () => {
    ...
    const [url, init] = mockFetch.mock.calls[0]!;
    expect(url).toBe('https://test-api.example.com/api/v1/deliberation/red-team');
    expect(init.method).toBe('POST');
    const body = JSON.parse(init.body);
