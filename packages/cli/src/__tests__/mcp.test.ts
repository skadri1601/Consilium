...
describe('mcpCommand --json', () => {
  ...
  it('includes CONSILIUM_API_URL in JSON env block', () => {
    ...
    expect(parsed.mcpServers.consilium.env.CONSILIUM_API_URL).toBe('https://api.myconsilium.xyz');
  });
  ...
  it('does not include the actual API key in JSON template', () => {
    ...
    const parsed = JSON.parse(logs.join('\n'));
    expect(parsed.mcpServers.consilium.env.CONSILIUM_API_KEY).toBe('${CONSILIUM_API_KEY}');
  });
});
