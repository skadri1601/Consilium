...
describe('listSessions', () => {
  ...
  it('returns sessions sorted by updatedAt descending', () => {
    writeSessionFile('ses_old', makeSessionData({
      id: 'ses_old',
      name: 'Old',
      updatedAt: '2025-01-01T00:00:00.000Z',
      debates: [{ topic: 'Old topic', timestamp: '2025-01-01T00:00:00.000Z' }],
    }));
    writeSessionFile('ses_new', makeSessionData({
      id: 'ses_new',
      name: 'New',
      updatedAt: '2025-06-01T00:00:00.000Z',
      debates: [{ topic: 'New topic', timestamp: '2025-06-01T00:00:00.000Z' }],
    }));
...
describe('loadSession', () => {
  ...
  it('returns ChatSession for existing session', () => {
    writeSessionFile('ses_load', makeSessionData({
      id: 'ses_load',
      name: 'Load Test',
      models: ['gpt-5.4'],
      mode: 'council',
...
describe('saveSession', () => {
  it('creates session file', () => {
    const session = makeFakeSession('ses_save1', 'Saved Session', [
      { topic: 'Test topic', goldenPrompt: 'result', timestamp: new Date().toISOString() },
    ]);
  ...
  it('updates existing session file', () => {
    ...
    const content = fs.readFileSync(path.join(SESSION_DIR, 'ses_update.json'), 'utf-8');
...
describe('renameSession', () => {
  it('updates session name in metadata', () => {
    writeSessionFile('ses_rename', makeSessionData({ id: 'ses_rename', name: 'Old Name' }));
    const result = sm.renameSession('ses_rename', 'New Name');
    expect(result).toBe(true);
    const content = fs.readFileSync(path.join(SESSION_DIR, 'ses_rename.json'), 'utf-8');
    const data = JSON.parse(content);
  ...
  it('updates the updatedAt timestamp', () => {
    writeSessionFile('ses_rename2', makeSessionData({
      id: 'ses_rename2',
      updatedAt: '2020-01-01T00:00:00.000Z',
    }));
    sm.renameSession('ses_rename2', 'Renamed');
    const content = fs.readFileSync(path.join(SESSION_DIR, 'ses_rename2.json'), 'utf-8');
    const data = JSON.parse(content);
...
describe('searchSessions', () => {
  ...
  it('finds sessions by topic keyword', () => {
    writeSessionFile('ses_search', makeSessionData({
      id: 'ses_search',
      name: 'Search Test',
      debates: [
        { topic: 'How to optimize database queries', timestamp: '2025-01-01T00:00:00.000Z' },
        { topic: 'React component patterns', timestamp: '2025-01-02T00:00:00.000Z' },
      ],
    }));
  ...
  it('finds sessions by synthesis content', () => {
    writeSessionFile('ses_synth', makeSessionData({
      id: 'ses_synth',
      name: 'Synth Test',
      debates: [
        {
  ...
  it('search is case-insensitive', () => {
    writeSessionFile('ses_case', makeSessionData({
      id: 'ses_case',
      name: 'Case Test',
      debates: [{ topic: 'TypeScript Generics', timestamp: '2025-01-01T00:00:00.000Z' }],
    }));
  ...
  it('returns empty for non-matching query', () => {
    writeSessionFile('ses_nomatch', makeSessionData({
      id: 'ses_nomatch',
      debates: [{ topic: 'Python decorators', timestamp: '2025-01-01T00:00:00.000Z' }],
    }));
