...
describe('getGitDiff', () => {
  it('returns combined staged and unstaged diffs', () => {
    mockedExecSync.mockImplementation((cmd: string) => {
      if ((cmd as string).includes('--cached')) return 'staged diff content' as any;
      if ((cmd as string).includes('git diff')) return 'unstaged diff content' as any;
...
describe('collectGitContext', () => {
  ...
  it('returns branch, diff, and commits when in a repo', () => {
    mockedExecSync.mockImplementation((cmd: string) => {
      ...
      if ((cmd as string).includes('branch --show-current')) return 'main' as any;
  ...
  it('returns null diff when no changes', () => {
    mockedExecSync.mockImplementation((cmd: string) => {
      if ((cmd as string).includes('rev-parse')) return 'true' as any;
      if ((cmd as string).includes('branch --show-current')) return 'dev' as any;
...
describe('formatGitContextForPrompt', () => {
  it('includes branch name', () => {
    const ctx: GitContext = { branch: 'feature/x', diff: null, recentCommits: null };
    const result = formatGitContextForPrompt(ctx);
    expect(result).toContain('Branch: feature/x');
  });

  it('includes recent commits', () => {
    const ctx: GitContext = { branch: null, diff: null, recentCommits: 'abc fix bug' };
    const result = formatGitContextForPrompt(ctx);
  ...
  it('includes diff content', () => {
    const ctx: GitContext = { branch: null, diff: 'some diff', recentCommits: null };
    const result = formatGitContextForPrompt(ctx);
  ...
  it('truncates large diffs at 30000 chars', () => {
    ...
    const ctx: GitContext = { branch: null, diff: largeDiff, recentCommits: null };
    const result = formatGitContextForPrompt(ctx);
