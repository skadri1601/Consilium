...
vi.mock('../utils/terminal-capabilities.js', () => ({
  terminal: { width: 80, isTTY: true, hasColor: false, hasUnicode: true, isCI: false, usePlain: false, isWindows: false },
  boxChars: () => ({ horizontal: '─', vertical: '│', topLeft: '┌', topRight: '┐', bottomLeft: '└', bottomRight: '┘' }),
}));
...
describe('visibleMatches', () => {
  ...
  it('filters by query', () => {
    ...
    expect(matches.every(m => m.name.includes('he') || m.summary.toLowerCase().includes('he'))).toBe(true);
  });
...
describe('renderFrame', () => {
  ...
  it('excludes palette content for non-slash buffer', () => {
    const state: PaletteState = { buffer: 'hello', paletteIndex: 0, prevLines: 0 };
    const { frame } = renderFrame(state);
...
describe('clampPaletteIndex', () => {
  it('clamps index to match count - 1 when exceeding', () => {
    const state: PaletteState = { buffer: '/', paletteIndex: 999, prevLines: 0 };
  ...
  it('resets to 0 when no matches', () => {
    const state: PaletteState = { buffer: '/zzzzzzzznotacommand', paletteIndex: 3, prevLines: 0 };
    clampPaletteIndex(state);
    expect(state.paletteIndex).toBe(0);
  });
