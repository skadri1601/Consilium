...
describe('ConsiliumClient.runStreamWithReconnect (via streamDebate)', () => {
  ...
  it('resolves when a done event arrives', async () => {
    ...
    es.onmessage?.({ data: JSON.stringify({ event: 'agent_chunk', chunk: 'hi' }), lastEventId: '1' } as MessageEvent);
    es.onmessage?.({ data: JSON.stringify({ event: 'done' }), lastEventId: '2' } as MessageEvent);
  ...
  it('sends Last-Event-ID on reconnect after transient drop', async () => {
    ...
    first.onmessage?.({ data: JSON.stringify({ event: 'agent_chunk' }), lastEventId: '42' } as MessageEvent);
    ...
    second!.onmessage?.({ data: JSON.stringify({ event: 'done' }) } as MessageEvent);
    await expect(promise).resolves.toBeUndefined();
  });

  it('does not retry on fatal 401 status', async () => {
    const client = new ConsiliumClient();
    const promise = client.streamDebate('dbt_1', () => undefined).catch((e) => e);
    await Promise.resolve();
