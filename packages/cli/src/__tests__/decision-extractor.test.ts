...
describe('DecisionLog', () => {
  ...
  describe('toJSON / fromJSON', () => {
    ...
    it('round-trips log with decisions', () => {
      ...
      const extraction: SemanticExtractionResult = {
        decisions: [{ decision: 'Use JWT', confidence: 0.9, supporting_models: ['gpt-4o'], category: 'AUTH' }],
      ...
      expect(restored.decisions[0].supportingModels).toEqual(['gpt-4o', 'claude-3']);
  ...
  describe('getContext', () => {
    ...
    it('includes decisions grouped by status', () => {
      log.decisions.push(
        { category: 'AUTH', statement: 'Use JWT', confidence: 'high', source: 't', debateIndex: 0, status: 'decided' },
        { category: 'DB', statement: 'Unclear', confidence: 'low', source: 't', debateIndex: 0, status: 'open' },
      );
