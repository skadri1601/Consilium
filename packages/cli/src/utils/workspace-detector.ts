...
export function detectWorkspace(dir?: string): WorkspaceInfo {
  ...
  if (fileExists(root, 'package.json')) {
  ...
  } else if (fileExists(root, 'pyproject.toml') || fileExists(root, 'requirements.txt')) {
    ...
    info.projectType = 'go';
    info.language = 'go';
  } else if (fileExists(root, 'pom.xml') || fileExists(root, 'build.gradle')) {
  ...
  info.hasDocker = fileExists(root, 'Dockerfile') || fileExists(root, 'docker-compose.yml');

  info.hasCI = dirExists(root, '.github/workflows') || fileExists(root, '.gitlab-ci.yml') || dirExists(root, '.circleci');
...
export function getAutoLoadFiles(info: WorkspaceInfo, dir?: string): string[] {
  ...
  switch (info.projectType) {
    case 'node':
      if (fileExists(root, 'package.json')) files.push(path.join(root, 'package.json'));
      if (fileExists(root, 'tsconfig.json')) files.push(path.join(root, 'tsconfig.json'));
      break;
    case 'python':
      if (fileExists(root, 'pyproject.toml')) files.push(path.join(root, 'pyproject.toml'));
      else if (fileExists(root, 'requirements.txt')) files.push(path.join(root, 'requirements.txt'));
      break;
    case 'rust':
      if (fileExists(root, 'Cargo.toml')) files.push(path.join(root, 'Cargo.toml'));
      break;
    case 'go':
...
export function formatWorkspaceInfo(info: WorkspaceInfo): string {
  ...
  if (info.packageManager !== 'unknown') lines.push(`Package Manager: ${info.packageManager}`);
  if (info.keyFiles.length > 0) lines.push(`Key Files: ${info.keyFiles.join(', ')}`);
  lines.push(`Tests: ${info.hasTests ? 'yes' : 'no'}`);
  lines.push(`Docker: ${info.hasDocker ? 'yes' : 'no'}`);
  lines.push(`CI: ${info.hasCI ? 'yes' : 'no'}`);
  return lines.join('\n');
}
