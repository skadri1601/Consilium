...
describe('project-memory', () => {
  ...
  it('appends a single entry and round-trips it', () => {
    ...
    appendProjectMemory(root, {
      ...
      mode: 'council',
      summary: 'Council recommends Neon for serverless scaling. Trade-off: vendor lock-in.',
      debateId: 'dbt_abc123',
    });
  ...
  it('appends multiple entries in chronological order', () => {
    ...
    appendProjectMemory(root, { topic: 'First', mode: 'quick', summary: 'one' });
    appendProjectMemory(root, { topic: 'Second', mode: 'auto', summary: 'two' });
    appendProjectMemory(root, { topic: 'Third', mode: 'council', summary: 'three' });
  ...
  it('truncates oversized summaries when reading back', () => {
    ...
    appendProjectMemory(root, { topic: 'Long', mode: 'deep', summary: longText });
