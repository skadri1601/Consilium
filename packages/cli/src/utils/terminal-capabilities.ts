...
const hasColor = isTTY && (process.env.FORCE_COLOR === '1' || process.env.FORCE_COLOR === 'true' || (typeof (process.stdout as { hasColors?: (count?: number) => boolean }).hasColors === 'function' && (process.stdout as { hasColors: (count?: number) => boolean }).hasColors(16)));
...
export const terminal = {
  ...
  usePlain: isCI || !isTTY,
  /** Width; fallback 80 */
  width: (process.stdout?.columns && process.stdout.columns > 0) ? Math.min(process.stdout.columns, 120) : 80,
};
