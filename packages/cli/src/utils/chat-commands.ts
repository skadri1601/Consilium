...
export function handleConversationsCommand(sessionManager: SessionManager): void {
  ...
  for (const sess of sessions.slice(0, 20)) {
    const id = sess.id.length > idW - 2 ? sess.id.slice(0, idW - 3) + '..' : sess.id;
    const topic = sess.topic.length > topicW - 2
...
export function handleModeCommand(
  ...
  if (args.length === 0) {
    ...
    if (config) {
      ...
      console.log(contentLine(`  Rounds: ${config.rounds}  Sub-agents: ${config.subAgents ? 'yes' : 'no'}`, w));
    }
    console.log(contentLine('', w));
    ...
    for (const [name, cfg] of Object.entries(DEBATE_MODES)) {
      ...
      console.log(contentLine(`  ${marker} ${name.padEnd(10)} ${cfg.description}`, w));
    }
    console.log(borderBottom(w));
...
export function handleCancelCommand(): { shouldCancel: boolean } {
  console.log(s.warning('  Cancellation requested. The current debate will be stopped.'));
  return { shouldCancel: true };
}

export function handleSkipCommand(): { shouldSkip: boolean } {
  console.log(
    s.warning('  Skipping remaining rounds. Judge will synthesize from available responses.')
  );
...
export function handleOutputCommand(
  ...
  const validFormats: OutputFormat[] = ['markdown', 'cursorrules', 'claude-md', 'json', 'text'];
