...
const PYTHON_ENTRY_CANDIDATES = ['main.py', 'app.py', 'manage.py', 'src/main.py', 'wsgi.py', 'asgi.py'];
...
function resolveFromManifests(manifests: Map<string, string>): ManifestMeta {
  if (manifests.has('package.json')) {
    ...
    return { projectType: 'node', language: result.language, framework: result.framework };
  }
  if (manifests.has('pyproject.toml') || manifests.has('setup.py')) {
    const content = manifests.get('pyproject.toml') || manifests.get('setup.py') || '';
    return { projectType: 'python', language: 'python', framework: detectPythonFramework(content) };
  }
