...
const DIR_PATTERN_RULES: Array<{ dirs: string[]; pattern: string }> = [
  { dirs: ['controllers', 'models', 'views', 'controller', 'model', 'view'], pattern: 'MVC' },
...
function detectDockerPattern(filePaths: string[], dirs: Set<string>, patterns: string[]): void {
...
function detectServerlessPattern(sourceFiles: Map<string, string>, filePaths: string[]): boolean {
  for (const [, content] of sourceFiles) {
    if (content.includes('serverless') || content.includes('lambda') || content.includes('@aws-cdk')) {
      return true;
    }
  }
  return filePaths.some((f) => f.includes('serverless.yml') || f.includes('serverless.ts'));
}

function detectPatterns(sourceFiles: Map<string, string>, filePaths: string[]): string[] {
  const dirNames = new Set(filePaths.flatMap((f) => path.dirname(f).split(path.sep)));
...
function collectSymbolNames(content: string, regex: RegExp, sink: Set<string>): void {
  regex.lastIndex = 0;
  let match;
...
const DATA_FLOW_RULES: Array<{ needles: string[]; label: string }> = [
  ...
  { needles: ['createConnection', 'mongoose', 'prisma', 'sequelize'], label: 'database' },
