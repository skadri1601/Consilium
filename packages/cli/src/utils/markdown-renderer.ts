...
function tryHeading(line: string): string | undefined {
  ...
  if (/^#\s+/.test(line)) return chalk.bold.underline(line.replace(/^#\s+/, ''));
  return undefined;
}
...
function consumeCodeBlockLine(line: string, s: FenceState, out: string[]): void {
...
function applyInlineStyles(text: string): string {
  let result = text;
  result = result.replaceAll(INLINE_CODE, (_, code: string) => chalk.bgGray.white(` ${code} `));
