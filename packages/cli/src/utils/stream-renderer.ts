...
function renderAgentCard(card: AgentCardState, w: number): string[] {
  ...
  const bodyLines = contentLines.slice(0, 20).map((line) => contentLineRounded(line || ' ', w));
  ...
  const durationSec = card.durationMs === undefined ? undefined : card.durationMs / 1000;
...
export function createStreamHandlers(options: StreamRenderOptions = {}) {
  ...
  function buildFrame(): string {
    ...
    const parts: string[] = [
      ...
      contentLine(`  Topic: ${(topic || '…').slice(0, 50)}${topic.length > 50 ? '…' : ''}`, w),
    ...
    if (consensusText) {
      const synLines = consensusText.split(/\n/).map((line) => contentLine(line || ' ', w));
      parts.push(st.dim(border('Synthesis', w)), borderLine(w), ...synLines, st.dim(borderBottom(w)) + '\n');
    }
  ...
  function onAgentChunk(event: DebateEvent): void {
    if (!event.text || currentIndex < 0 || currentIndex >= agents.length) return;
  ...
  function onAgentComplete(): void {
    const card = currentIndex >= 0 && currentIndex < agents.length ? agents[currentIndex] : undefined;
    if (card) {
      card.status = 'done';
