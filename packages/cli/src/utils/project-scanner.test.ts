...
describe('scanProject', () => {
  ...
  it('scans nested directories and excludes secrets', () => {
    ...
    fs.writeFileSync(path.join(root, 'apps', 'web', 'src', 'index.ts'), 'export const ok = true;\n', 'utf-8');
    fs.writeFileSync(path.join(root, '.env'), 'SECRET=123\n', 'utf-8');

    const result = scanProject(root, { maxFiles: 50, maxTotalBytes: 1024 * 1024, maxDepth: 10 });
    expect(result.files.some((f) => f.path === 'apps/web/src/index.ts')).toBe(true);
