...
export function contentLine(text: string, width: number = terminal.width): string {
  const bc = boxChars();
...
export function borderRounded(title: string, width: number = terminal.width): string {
...
export function contentLineRounded(text: string, width: number = terminal.width): string {
  const bc = boxChars('rounded');
  const w = Math.max(0, width - 4);
  const truncated = text.length <= w ? text : text.slice(0, w - 3) + '…';
  return `${bc.vertical} ${truncated.padEnd(w - 1)} ${bc.vertical}`;
