...
const KEYWORDS_JS = new Set([
  ...
  'case', 'break', 'continue', 'default', 'interface', 'type', 'enum',
]);

const BUILTINS_JS = new Set([
  'console', 'require', 'process', 'module', 'exports', 'Promise',
...
const BUILTINS_PY = new Set([
  ...
  'float', 'bool', 'type', 'isinstance', 'super', 'self',
]);
...
export function detectLanguage(code: string): string {
  ...
  if (/\bdef\s+\w+\s*\(/.test(trimmed) || (/\bimport\s+/.test(trimmed) && !trimmed.includes(';'))) return 'python';
  if (/\b(function|const|let|var)\b/.test(trimmed) || /=>/.test(trimmed)) return 'javascript';
  return 'unknown';
}
