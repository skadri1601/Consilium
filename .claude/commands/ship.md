...
allowed-tools: Bash(pnpm:*), Bash(poetry run:*), Bash(git status:*), Bash(git diff:*), Bash(git diff --stat:*)
---
...
3. Type-check the affected package(s), e.g. `pnpm --filter @consilium/web type-check`, `pnpm --filter @consilium/api type-check`. For agents: `cd apps/agents && poetry run ruff check src/`.
4. Tests for the affected package(s):
   - web: `pnpm --filter @consilium/web test -- --run`
   - api: `pnpm --filter @consilium/api test`
   - agents: `cd apps/agents && poetry run pytest tests/deliberation/ -x`
